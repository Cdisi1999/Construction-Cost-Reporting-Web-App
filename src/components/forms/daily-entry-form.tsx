...
export function DailyEntryForm({ costCodes }: { costCodes: CostCodeOption[] }) {
  ...
  const laborTotals = useMemo(() => {
    ...
    const headcount = laborers.filter((laborer) => laborer.name.trim().length > 0 || Number(laborer.hours || 0) > 0).length;
  ...
  async function submitForm(event: React.FormEvent) {
    ...
    const response = await fetch("/api/entries", {
      ...
      body: JSON.stringify({
        ...form,
        labor_entries: laborers
          .filter((entry) => entry.name.trim() || Number(entry.hours || 0) > 0)
          .map((entry) => ({ laborer_name: entry.name.trim(), hours: Number(entry.hours || 0) })),
          ...
          <div>
            ...
            <select
              ...
              onChange={(event) => {
                ...
                const selected = costCodes.find((option) => option.code === selectedCode);
                setForm((prev) => ({
                  ...prev,
            ...
            {!hasCostCodes ? (
              <p className="mt-1 text-xs text-amber-700">Cost Codes.csv was not found or has no valid rows.</p>
            ) : null}
          </div>
          <div>
